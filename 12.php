...
function createMinCost(array $costs)
{
    return function (array $items) use ($costs) {
        return min(
            array_map(function ($cost) use ($items) {
                return $cost($items);
            }, $costs));
    };
}
...
$monthCost = createCriteriaCost($priceBetweenCost, function () {
    return date('d') == 15;
}, 5);
$birthdayCost = createCriteriaCost($priceBetweenCost, function () {
    return date('m-d') == '08-12';
}, 6);
